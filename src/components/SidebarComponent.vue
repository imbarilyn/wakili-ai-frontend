...
<!--   sidenav on large screen-->
    <div class="hidden  lg:block top-0 bottom-0 h-screen">
      <div
        :class="[chatbotStore.collapseSidebarOnLarge? 'w-[64px] duration-1000 overflow-y-hidden overflow-x-hidden' : 'duration-700']"
        class="relative w-64 flex h-full flex-col">
        <div
          :class="[chatbotStore.collapseSidebarOnLarge? 'w-[64px] duration-1000 overflow-y-hidden overflow-x-hidden' : 'duration-700']"
          class="fixed py-4 px-4 bg-secondary-color w-64 h-screen">
          <div :class="[chatbotStore.collapseSidebarOnLarge? '': '']">
            <button class="btn btn-sm btn-ghost" @click="chatbotStore.setCollapseSidebarOnLarge()" :class="[chatbotStore.collapseSidebarOnLarge? 'px-1': '']">
              <span class="material-icons-outlined text-main-color !text-2xl">menu</span>
            </button>
          </div>
          <div class="pt-10 group">
            <button
              :class="[chatbotStore.collapseSidebarOnLarge? 'btn-circle flex': '']"
              ...
              <span class="material-icons-outlined" :class="[showNewChat? 'text-main-color': 'text-white']" >add</span>
              <span class="font-normal" v-if="!chatbotStore.collapseSidebarOnLarge" :class="[showNewChat? 'text-main-color': 'text-white ']">New Chat</span>
            </button>
            <div class="absolute left-64 pb-2 bg-main-color text-xs text-white rounded-lg whitespace-pre drop-shadow-lg px-0 py-0 w-0 group-hover:px-0.5 group-hover:py-0.5 group-hover:left-2 group-hover:mt-3   group-hover:duration-700 group-hover:w-fit  " v-if="chatbotStore.collapseSidebarOnLarge">
              <p>New Chat</p>
            </div>
          </div>
          <div v-if="!chatbotStore.collapseSidebarOnLarge && chatbotStore.chatHistoryTitle" class="overflow-y-auto max-h-[480px] mt-4">
            <div v-for="(chats, date) in groupChatbyDate()" :key="date">
              <h1 class="font-bold sticky top-0 backdrop-blur pb-2">{{date}}</h1>
              <div v-for="chat in chats" :key="chat.id" class="px-2" @click="setActiveHistoryButton(chat.conversationId)">
                <ChatHistory
            ...
            <div class="absolute bottom-6 flex flex-col w-52">
              ...
              <div class="relative  group flex flex-col py-3">
                <button
                  ...
                  :class="[chatbotStore.collapseSidebarOnLarge? 'btn-circle btn-ghost flex justify-center text-sm': '', showSubscription? 'bg-secondary-color shadow-4xl': '']"
                  class="btn btn-sm flex duration-500 justify-start mt-2 bg-main-color">
                  <span class="material-icons-outlined te" :class="[showSubscription? 'text-main-color': 'text-white']">subscriptions</span>
                  <span class="font-normal"  :class="[showSubscription? 'text-main-color': 'text-white']" v-if="!chatbotStore.collapseSidebarOnLarge">Subscriptions</span>
                </button>
<!--                <div-->
<!--                  :class="[!collapseSidebarOnLarge?'hidden': '']"-->
<!--                  class="absolute left-64 pb-2 bg-main-color text-xs text-white rounded-lg whitespace-pre drop-shadow-lg px-0 py-0 w-0 group-hover:px-0.5 group-hover:py-0.5 group-hover:left-2 group-hover:mt-3   group-hover:duration-700 group-hover:w-fit  " v-if="showSubscription">-->
<!--                  <p class="text-nowrap text-xs">Subscription</p>-->
<!--                </div>-->
              </div>
<!--              <div>-->
...
<!--              </div>-->
              ...
              <div class="relative flex flex-col">
                <div class="relative group flex flex-col">
                  <button
                    ...
                    :class="[chatbotStore.collapseSidebarOnLarge? 'btn-circle btn-ghost flex justify-center text-sm': '', showLogout? 'bg-secondary-color shadow-4xl': '']"
                    class="btn btn-sm flex duration-500 justify-start mt-2 bg-main-color">
                    <span class="material-icons-outlined text-sm" :class="[showLogout? 'text-main-color': 'text-white']">logout</span>
                    <span class="font-normal" :class="[showLogout? 'text-main-color': 'text-white']"  v-if="!chatbotStore.collapseSidebarOnLarge">Logout</span>
                  </button>
                  <div
                    :class="[!chatbotStore.collapseSidebarOnLarge?'hidden': '']"
          ...
          <div class="absolute bottom-0 flex" v-if="!chatbotStore.collapseSidebarOnLarge">
                  ...
                  <DialogPanel class="w-64 pointer-events-auto h-screen relative top-0 bottom-0 overflow-y-hidden" id="SidebarDialog">
                    <div class="flex h-screen flex-col overflow-y-auto bg-secondary-color py-4 shadow-xl">
                      ...
                      <div class="pt-6 flex-1 px-2 sm:px-4">
                        <div class="relative">
                          <div>
                            <button
                              @mouseover="showNewChat=true"
                              @mouseleave="showNewChat=false"
                              :class="[showNewChat? 'bg-secondary-color shadow-4xl': '']"
                              class="btn btn-sm btn-ghost rounded-full bg-main-color" @click.stop="newChat" >
                              <span class="material-icons-outlined " :class="[showNewChat? 'text-main-color': 'text-white']"> add</span>
                              <span class="font-normal" :class="[showNewChat? 'text-main-color': 'text-white']">New Chat</span>
                            </button>
                          </div>
                        ...
                        <div class="absolute bottom-6 flex flex-col w-52 space-y-2">
                          <button
                            @click="subscribePage"
                            @mouseover="showSubscription=true"
                            @mouseleave="showSubscription=false"
                            :class="[showSubscription? 'bg-secondary-color shadow-4xl': '']"
                            class="btn btn-sm flex duration-500 justify-start mt-2 bg-main-color hover:text-main-color">
                            <span class="material-icons-outlined" :class="[showSubscription? 'text-main-color': 'text-white' ]">subscriptions</span>
                            <span class="font-normal" :class="[showSubscription? 'text-main-color': 'text-white' ]">Subscription</span>
                          </button>
...
<!--                            </button>-->
                          ...
                          <button class="btn btn-sm flex justify-start bg-main-color"
                                  ...
                                  @mouseleave="showLogout = false"
                                  @click="logout"
                                  :class="[showLogout? 'bg-secondary-color shadow-4xl': '']"
                          >
                            <span class="material-icons-outlined" :class="[showLogout? 'text-main-color': 'text-white' ]">logout</span>
                            <span class="font-normal" :class="[showLogout? 'text-main-color': 'text-white' ]">Logout</span>
                          </button>
                        </div>
